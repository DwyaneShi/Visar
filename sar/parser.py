...
from sar import PART_CPU, PART_MEM, PART_SWP, PART_IO, PART_PAGING, PART_NET, PART_DEV, \
    PATTERN_CPU, PATTERN_MEM, PATTERN_SWP, PATTERN_IO, PATTERN_PAGING, \
    PATTERN_NET, PATTERN_DEV, PATTERN_RESTART, PATTERN_HOSTNAME, \
    ...
    FIELDS_NET, FIELD_PAIRS_NET, FIELDS_DEV, FIELD_PAIRS_DEV
import mmap
import os
import re
import traceback
...
class Parser(object):
    ...
    def load_file(self):
        ...
        if searchunks:
            ...
            host_name, cpu_usage, mem_usage, swp_usage, io_usage, paging_stats,\
            net_usage, dev_usage = \
    ...
    def _parse_file(self, sar_parts):
        ...
        if type(sar_parts) is ListType:
            ...
            host_pattern = re.compile(PATTERN_HOSTNAME)
            ...
            dev_pattern = re.compile(PATTERN_DEV)
            ...
            for part in sar_parts:
                logging.debug(part)
                if host_pattern.search(part):
                    if host_name == '':
                        host_name = host_pattern.findall(part)[0]
                ...
                # Try to match device usage SAR file sections
                if dev_pattern.search(part):
                    if dev_usage == '':
                        dev_usage = part
                        try:
                            first_line = part.split("\n")[0]
                        ...
                        self.__dev_fields = \
                        self.__find_column(FIELDS_DEV, first_line)
                    else:
                        dev_usage += "\n" + part
            ...
            dev_output = self.__split_info(dev_usage, PART_DEV)
            ...
            return (host_name, cpu_output, mem_output, swp_output, io_output,
                    paging_output, net_output, dev_output)
                ...
                if full_time != "Average:":
                    ...
                    for sectionname in pairs.iterkeys():
                        ...
                        if part_type == PART_CPU:
                            ...
                            dev = elems[(1 if is_24hr is True else 2)]
                            try:
                                blah = return_dict[full_time][dev]
                                del(blah)
                            except KeyError:
                                return_dict[full_time][dev] = {}
                            return_dict[full_time][dev][sectionname] = \
                                value
                        else:
